...
# Fixed run set requested by user (timestamp:nodes).
RUNS=(
...
# Collectives present across all runs above.
DEFAULT_COLLECTIVES=(
  "ALLGATHER"
  "ALLREDUCE"
  "BCAST"
...
# Ensure every summary exists before plotting.
for run in "${RUNS[@]}"; do
  ts="${run%%:*}"
...
for ppn in 1 4; do
  for collective in "${TARGET_COLLECTIVES[@]}"; do
    ...
    mkdir -p "${outdir}"
    outfile_base="${outdir}/${SYSTEM}_${collective_lower}_default_vs_best_${METRIC}_${ppn}ppn.pdf"
    outfile_no_bine="${outdir}/${SYSTEM}_${collective_lower}_default_vs_best_${METRIC}_${ppn}ppn_no_bine.pdf"

    common_cmd=(
      python3 -m plot default-vs-best-heatmap
      --system "${SYSTEM}"
    ...
    echo "Generating ${collective} (${ppn} ppn) -> ${outfile_base}"
    "${base_cmd[@]}"
    ...
    echo "Generating ${collective} (${ppn} ppn, no-bine) -> ${outfile_no_bine}"
    "${no_bine_cmd[@]}"
  done
done
